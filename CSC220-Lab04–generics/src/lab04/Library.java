...
public class Library<Type> {
	...
	public ArrayList<LibraryBook<Type>> getOrderedByAuthor() {
		...
		ArrayList<LibraryBook<Type>> libraryCopy = new ArrayList<LibraryBook<Type>>();
		// Assign a value to the library
		libraryCopy.addAll(library);
		
		// Declare an AuthorComparator object
	...
	public ArrayList<LibraryBook<Type>> getOverdueList(int month, int day, int year) {
		...
		for (LibraryBook<Type> item : library) {
			// Duedate cannot be empty and is greater than the specified date
			if (item.getDueDate() != null && item.getDueDate().compareTo(calendar) > 0) {
				libraryCopy.add(item);
			}
		}
		// Declare an AuthorComparator object
	...
	protected class AuthorComparator implements Comparator<LibraryBook<Type>> {
		...
		@Override
		public int compare(LibraryBook<Type> o1, LibraryBook<Type> o2) {
			// Compare authors for equality
			if (o1.getAuthor().equals(o2.getAuthor())) {
				// If two books have the same author, this Comparator should break the tie with
				// the book title
